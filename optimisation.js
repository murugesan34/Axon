  ...
  app.update = function (timestamp) {
    for (var i = 0; i < app.count; i++) {
      ...
      if (!app.optimize) {
        ...
        // Optimised code here
        /**
        *  offsetTop  properities I replaced with top
        *  and then finding the image position and checking the image class name have 'down'
        *  if its true then increase the image position with distance else decrease the position with distance and assign to the 
        *  variable imagePosition
        *  after that checked imagePosition is greated than maxHeight or less then 0
        *  based on that add / remove the class name up & down in that images.
        */
        var imagePosition = parseInt(m.style.top.slice(0, m.style.top.indexOf('px')));
        m.classList.contains('down') ? imagePosition += distance : imagePosition -= distance;
        if (imagePosition < 0) imagePosition = 0;
        if (imagePosition > maxHeight) imagePosition = maxHeight;
        m.style.top = imagePosition + 'px';
        if (imagePosition == 0) {
          m.classList.remove('up');
          m.classList.add('down');
        }
        if (imagePosition == maxHeight) {
          m.classList.remove('down');
          m.classList.add('up');
        }
      }
